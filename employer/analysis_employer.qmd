...
cloud3 <- trigram %>%
  ...
  wordcloud(words = cloud3$trigram, 
            freq = cloud3$n, 
            min.freq = 50,
            max.words=100,
            random.order=FALSE, 
...
df %>% 
  ...
  labs( title = "Languages for Analyst Jobs",
        y = "Languages")
```
## Programming languages mentioned by employers (Universities)
```{r university}
df %>% 
...
### LDA models onto Employer Data
```{r lda_stat}
lda_model10_s <- readRDS("lda_model10_s.rda")

topics_stat <- tidy(lda_model10_s, matrix = "beta")

emp_word_stat <- emp_word %>% 
  left_join(topics_stat, by = c("stem" = "term") ) %>% 
  group_by(jobTitle, jobClassification, advertiserName, workType,word, stem) %>% 
  top_n(1, beta) %>% 
  mutate(topic = as.factor(topic))

#write.csv(emp_word_stat, file = "../presentation/data/emp_stat.csv")
```
