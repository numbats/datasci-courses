...
```{.r code-line-numbers="1-2|4-5|7-9|11-20|22-23|25-28"}
library(RSelenium)
library(rvest)
...
- Due to time limit we have only fitted a fairly basic Latent Dirichlet Allocation (LDA) model using `text2vec` package
  ...
  - Distances between words are then computed, with the assumption that words are 'close' to each other are similar and shall be grouped together

- The model has to be trained by large amount of data pre-use

  - **4,448 Wikipedia articles** in *statistics (2,816), sociology (1,005) and computing (627)* are scrapped as training data
...
# Employer Perspectives {background-color="#006DAE"}
...
## Job Analysis {background-image="images/bg.jpg" background-opacity="0.5"}
