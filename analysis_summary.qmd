...
# write.csv(total_comb, file = "presentation/unidata.csv")
...
```{r}
targets::tar_load(lda_model10_s)
targets::tar_load(lda_model12_ssc)
targets::tar_load(lda_model10_ssc)

topics_ssc <- tidy(lda_model12_ssc, matrix = "beta")
topics_ssc2 <- tidy(lda_model10_ssc, matrix = "beta")
topics_s <- tidy(lda_model10_s, matrix = "beta")

#write.csv(topics_ssc, file = "presentation/data/modelsum_ssc.csv")
```
...
# write.csv(words_ssc12, file = "presentation/ssc.csv")

# write.csv(words_s, file = "presentation/stats.csv")
```
...
```{r ssc with lda_model 10}
words_ssc2 <- words %>% 
  left_join(topics_ssc2, by = c("stem" = "term") ) %>% 
  group_by(School, Course, Course_code, Unit, Unit_code, words, stem) %>% 
  top_n(1, beta) %>% 
  mutate(topic = as_factor(topic))
# write.csv(words_ssc2, file = "presentation/ssc.csv")
...
words_ssc2 %>% 
  ...
  facet_wrap(~School, scales = "free_y") +
  theme(legend.position = "none") +
  labs(title = "Topic Count - LDA_SSC2")

topicmodels::get_terms(targets::tar_read(lda_model10_ssc), k = 10)
```
...
words_s %>% 
  ...
  facet_wrap(~School, scales = "free_y") +
  theme(legend.position = "none") +
  labs(title = "Topic Count Overview - LDA_S")
...
words_s %>% 
  ...
  mutate(prop = round(n/sum(n),4)) %>% 
  ungroup() %>% 
  mutate(topic = fct_reorder(topic, prop, sum))%>% #reorder_within(topic, n, School)
  ggplot(aes(x = prop,
             y = topic,
             fill = topic)) +
  geom_col() +
  ...
  theme(legend.position = "none") +
  labs(x = "proportion",
       title = "Topic Proportion by University - LDA_S")

```


```{r}
targets::tar_load(lda_model10_comp)
...
write.csv(words_comp, file = "presentation/comp.csv")
```
...
```{r}
topicmodels::get_terms(targets::tar_read(lda_model10_comp), k = 10)

```


```{r}
targets::tar_load(lda_model10_cs)

topics_cs <- tidy(lda_model10_cs, matrix = "beta")

words_cs <- words %>% 
  left_join(topics_cs, by = c("stem" = "term") ) %>% 
  group_by(School, Course, Course_code, Unit, Unit_code, words, stem) %>% 
  top_n(1, beta) %>% 
  mutate(topic = as_factor(topic))

#write.csv(words_cs, file = "presentation/data/cs.csv")
```
...
words_cs %>% 
  ...
  mutate(prop = round(n/sum(n),4)) %>% 
  ungroup() %>% 
  mutate(topic = fct_reorder(as_factor(topic), n, sum))%>% 
  ggplot(aes(x = prop,
             y = topic,
             fill = topic))+
  geom_col() +
  ...
  facet_wrap(~School, scales = "free_y") +
  theme(legend.position = "none") +
  labs(title = "Topic Count - LDA_CS")
```


```{r}
#save model file
saveRDS(targets::tar_load(lda_model10_ssc),"presentation/lda_model10_ssc.rda")

saveRDS(targets::tar_load(lda_model10_comp),"employer/lda_model10_comp.rda")

saveRDS(targets::tar_load(lda_model10_s),"presentation/lda_model10_s.rda")

saveRDS(targets::tar_load(lda_model10_cs),"presentation/lda_model10_cs.rda")

```


```{r}
# save top 10 terms for each topic

term_cs <- topicmodels::get_terms(targets::tar_read(lda_model10_cs), k = 10) %>% 
  as_tibble()

term_s <- topicmodels::get_terms(targets::tar_read(lda_model10_s), k = 10) %>% 
  as_tibble()

term_s30 <- topicmodels::get_terms(targets::tar_read(lda_model10_s), k = 30) %>% 
  as_tibble()


term_ssc <- topicmodels::get_terms(targets::tar_read(lda_model10_ssc), k = 10) %>% 
  as_tibble()
  
term_comp <- topicmodels::get_terms(targets::tar_read(lda_model10_comp), k = 10) %>% 
  as_tibble()


#write.csv(term_comp, file = "presentation/data/term_comp.csv")
#write.csv(term_s30, file = "presentation/data/term_s30.csv")
```



## Bigram, university breakdown by proportion using combined overview and outcome 
